...
export default async function SingleStudy({ params }: { params: { id: string }}) {
  ...
  if (error || !data?.user) {
    ...
    const { data: singleStudy } = await supabase.from("studies").select().eq("namestudy", params.id).eq("email", data.user.email);
  ...
  return (
    <main className="w-full max-w-6xl text-xl animate-in">
      ...
      <div className="flex justify-center space-x-5 flex-row mb-5">
        ...
        <p className="font-bold">{params.id}</p> participants have joined your study
        </div>
        <div className="flex-col flex w-64">
        <p className="font-bold">{params.id}</p> concepts on average have been drawn
        </div>
        <div className="flex-col flex w-64">
        <p className="font-bold">{params.id}</p> is the mean valence of the drawn concepts
        </div>
        <div className="flex-col flex w-64">
        <p className="font-bold">{params.id}</p> was the last time you collected a CAM
        </div>
      </div>
...
bb
        ...
        <div className="flex items-center justify-center w-64 h-8 border bg-red-100">  aaa
        </div>
        <div className="flex items-center justify-center w-64 h-8 border bg-red-100">  bbb
        </div>
      </div>
    </main>
  );
